...
void solve() {
    ...
    if(n==1){
        prt(max(mat[0][0],mat[0][1]));
        return;
    }
    dp[0][0]=mat[0][0];
    dp[0][1]=mat[0][1];
    dp[1][0]=mat[1][0]+dp[0][1];
    dp[1][1]=mat[1][1]+dp[0][0];
    rep(i,2,n){
        ll val0=0,val1=0;
        val0=max(mat[i][0]+dp[1][1],mat[i][0]+dp[0][1]);
        val1=max(mat[i][1]+dp[1][0],mat[i][1]+dp[0][0]);
        dp[0][0]=dp[1][0];
        dp[0][1]=dp[1][1];
        dp[1][0]=val0;
        dp[1][1]=val1;
