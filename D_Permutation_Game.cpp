...
#define vin1(v, n) for (ll i = 1; i <= n; ++i) cin >> v[i]
...
void sieve(int n = 2000000) {
 ...
 void fun(vll vec,vll& path,ll ppb,vll per,vll a){
    while(true){
        if(vec[ppb]==1){
            break;
        }
...
void solve() {
 ...
 vll pathb;
 vll paths;
 fun(vec,pathb,ppb,per,a);
 fun(vec,paths,ps,per,a);
 ll m1=0;
 ll m2=0;
 fun2(m1,pathb,k);
 fun2(m2,paths,k);
