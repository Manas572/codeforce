...
void solve() {
    ...
    vin(vec,n);
    vll dp(2,0);
    dp[0]=max(vec[0],llabs(vec[0]));
    dp[1]=min(vec[0],llabs(vec[0]));  
    rep(i,1,n){
       vll temp(2,0);
       temp[0]=max(dp[0]+vec[i],llabs(dp[1]+vec[i]));
       temp[1]=min(dp[1]+vec[i],llabs(dp[0]+vec[i]));
       dp=temp;
    }
    prt(max(dp[0],llabs(dp[1])));
}
