...
pll fun(vll seen,map<ll,vll> mp,ll node){
    queue<pll> q;
    q.push({node,0});
    ll ansnode=0;
    ll anslvl=0;
    while (!q.empty())
    {
        ll pnode=q.front().first;
        ll plvl=q.front().second;
        ansnode=pnode;
        anslvl=plvl;
        seen[pnode]=1;
        q.pop();
    ...
    return make_pair(ansnode,anslvl);
}
// -------- Solve --------
void solve() {
    ll n;
    ...
    vll seen(n+2,0);
    auto p=fun(seen,mp,1);
    auto p2=fun(seen,mp,p.first);
    prt(p2.second);
}
