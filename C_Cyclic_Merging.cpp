...
ll findp(ll pos, vll& L, ll n) {
    if (L[pos] == pos) return pos; 
    return L[pos] = findp(L[pos], L, n);
}

ll findf(ll pos, vll& R, ll n) {
    if (R[pos] == pos) return pos; 
    return R[pos] = findf(R[pos], R, n); 
}
...
void solve() {
    ...
    for(int i = 0; i < n - 1; i++) {
        ll ind = vec[i].second;
        ll li = findp((ind - 1 + n) % n, L, n);
        ll ri = findf((ind + 1) % n, R, n);
        ll pval = a[li];
        ll fval = a[ri];
        c += min(pval, fval);
        L[ind] = li;
