...
void solve() {
    ...
    vll happiness(ts+10,1e12);
    happiness[0]=0;
    rep(i,1,m+1){
        ll totsal=(i-1)*s;
        ll amtp=vec[i-1].first;
        ll hiwg=vec[i-1].second;
        rev(j,happiness.size()-1,hiwg){
            ll amaspend=happiness[j-hiwg];
            if(amaspend+amtp<=totsal){
                chmin(happiness[j],amaspend+amtp);
            }
        }
    }
    rev(i,happiness.size()-1,0){
        if(happiness[i]!=1e12){
            prt(i);
            return;
        }
