...
ll fun(vll& vec,ll n,ll& m,ll p,vector<vll>& dp){
     //prt(n);
    if(n==0){
        dp[n][p]= 1;
        return dp[n][p];
   ...
   if(vec[n-1]!=0){
    ...
    dp[n][p]= fun(vec,n-1,m,vec[n-1],dp);
    return dp[n][p];
   }else{
    ll ans=0;
    ans=(ans+fun(vec,n-1,m,p,dp))%MOD;
    if(p<m){
        ans=(ans+fun(vec,n-1,m,p+1,dp))%MOD;
    }
    if(p>1){
        ans=(ans+fun(vec,n-1,m,p-1,dp))%MOD;
    }
...
void solve() {
    ...
    //vout(vec);
     if(vec[n-1]!=0){
        prt(fun(vec,n-1,m,vec[n-1],dp));
     }else{
        ll ans=0;
        rep(i,1,m+1){
            ans=(ans+fun(vec,n-1,m,i,dp))%MOD;
        }
        prt(ans);
     }
}
