...
void solve() {
    ...
    while (top <= bottom && left <= right) {
        for (ll i = left; i <= right; ++i) mat[top][i] = val--;
        top++;
 
        for (ll i = top; i <= bottom; ++i) mat[i][right] = val--;
        right--;

        if (top <= bottom) {
            for (ll i = right; i >= left; --i) mat[bottom][i] = val--;
            bottom--;
        }
        if (left <= right) {
            for (ll i = bottom; i >= top; --i) mat[i][left] = val--;
            left++;
        }
    }
